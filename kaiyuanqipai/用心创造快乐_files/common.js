...
f_com.logoff = function(loc) {
    ...
    xhttp.open('GET', '/infe/rest/walletrecycle/recycle?action=out', false);
    xhttp.send();
...
function MemberMsg() {
    try {
        ...
        $.ajax({
            type: 'POST',
            url: '/infe/macenter/message/usermsgcontrollerv2/getHomePageMsg',
            error: function(msg) {
                alert('error');
            },
            success: function(data) {
...
function HotNewsHistory() {
     ...
     window.open('/infe/marquee/historyNews', 'HotNewsHistory', features);
}
...
$.fn.flashing = function() {
    ...
    $(this).each(function() {
        var colorTable = $(this).data('color');

        if (typeof(colorTable) === 'undefined') return;

        colorTable = colorTable.split('|');
        if (colorTable.length == 2) {
            new toggleColor(this, [colorTable[0], colorTable[1]], 500);
        } else if (colorTable.length == 1 && colorTable[0] != '') {
            $(this).css('color', colorTable[0]);
        }
    });
 ...
 */
f_com.MGetPager = function(page) {
    // 新舊頁面代碼轉換
    // 為了日後功能擴充後方便搜尋取代，故不直接更換連結，而採用轉換的方式
    var pageTrans = {
        bankSavings: 'deposit',
        bankTake: 'withdraw',
        moneyView: 'moneySwitch',
        gameHistory: 'betRecord',
        liveHistory: 'betRecord',
        lotteryRecord: 'betRecord',
        msg: 'userMsg',
        ballRecord: 'betRecord',
        ballHistory: 'betRecord',
        skRecord: 'betRecord',
        skHistory: 'betRecord',
        userAccount: 'cashRecord'
    };

    window.open("/infe/mcenter/#/" + (pageTrans[page] || page), "MACENTER", "top=50,left=50,width=1020,height=768,status=no,scrollbars=yes,resizable=no").focus();
};
...
f_com.BBinGetPager = function(mo) {
    window.open("/infe/rule/" + mo, "?bb", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
};
...
f_com.BBinEnterPager = function(mo,me,ty) {
    //視窗寬度ub連覽器判斷
    if (!ty){
        window.open("/infe/rule/" + mo +"?PageItem=" + me , "BBinPage", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
    }else{
        window.open("/infe/rule/" + mo +"?PageItem=" + me+"&type=" + ty , "BBinPage", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
    }
};
/*開啟bbin資訊中心-指定頁面並無左側選單*/
f_com.BBinOnlyPager = function(mo,me,ty)  {
    if (!ty){
        window.open("/infe/rule/" + mo +"?OnlyPage=Y&PageItem=" + me , "BBinPage", "top=50,left=50,width=770,height=700,status=no,scrollbars=no,resizable=yes").focus();
        return;
    }

    window.open("/infe/rule/" + mo +"?PageItem=" + me+"&OnlyPage=Y&type=" + ty , "BBinPage", "top=50,left=50,width=770,height=700,status=no,scrollbars=no,resizable=yes").focus();
}
/*event活動開啟*/
...
/* 廣宣系統-連結另開處理/流量統計 */
f_com.advertise = function(options) {
    // 另開視窗
    f_com.bm(options.link, ((options.name === '') ? 'openWin' : options.name), options.openOption);

    // 流量統計
    if (options.analytics) {
        bbinAnalytics('廣宣', options.analyticsLocation, options.analyticsName);
    }
};

/* 電子廣宣 - 預設開啟設置 (取cookie值) */
f_com.getCasinoSliderPanel = function(gameId) {
    var getCookie = JSON.parse($.cookie("gameSlider"));

    if ($.cookie("gameSlider") !== null && typeof(getCookie['casino_' + gameId]) !== 'undefined') {
        return getCookie['casino_' + gameId];
    }

    return 1;
};

/* 電子廣宣 - 預設開啟設置 (設定cookie值) */
f_com.setCasinoSliderPanel = function(gameId, currentPanel) {
    var setCookie = ($.cookie("gameSlider") === null) ? {} : JSON.parse($.cookie("gameSlider"));

    setCookie['casino_' + gameId] = currentPanel;
    document.cookie = 'gameSlider=' + JSON.stringify(setCookie);
};
